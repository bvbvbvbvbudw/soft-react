...
const BackLogPage = () => {
        ...
        const fetchInitialData = async () => {
            try {
                const [tasksResponse, projectResponse] = await axios.all([
                    axios.get(`http://localhost:8000/api/currentTask?project_id=${projectId}`),
                    axios.get(`http://localhost:8000/api/projectadduser?project_id=${projectId}&user_id=${encodeURIComponent(localStorage.getItem('userName'))}`)
                ]);
    ...
    useEffect(() => {
        Promise.all(avatars.map(avatar => {
            return axios.get(`http://localhost:8000/api/avatarLoad?user_id=${avatar.id}`);
        }))
            .then(responses => {
    ...
    const deleteUserHandler = (id) => {
        console.log(id)
        axios.post(`http://localhost:8000/api/projectadduser?deleteId=${id}&project_id=${projectId}`)
            .then(response =>
    ...
    const handleChangeTaskInfo = (task, e) => {
        ...
        axios.post(`http://localhost:8000/api/task?id=${task.id}`, {
            name: task.name,
                        ...
                        <div className="content-table">
                            ...
                            {tasks.length > 0 ? (
                                tasks.map((task, index) => {
                                    ...
                                    const avatarSrc = taskAvatar ? `http://localhost:8000/storage/${taskAvatar.avatar}` : '';
