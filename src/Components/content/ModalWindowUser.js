...
function MyVerticallyCenteredModal(props) {
    ...
    const sendRequest = (e) => {
        ...
        e.preventDefault();
        axios
            .get(`http://localhost:8000/api/findUser?name=${inputReq.value}`)
            .then(response => {
                setAddUser(response.data);
                setTimeout(() => {
                    axios
                        .get(`http://localhost:8000/api/avatarLoad?user_id=${response.data[0].id}`)
                        .then(response => {
    ...
    const addUserHandler = (id) => {
        ...
        axios
            .post('http://localhost:8000/api/projectadduser',
                {
